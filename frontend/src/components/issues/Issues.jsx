        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setDarkMode(!darkMode)}
              className={`w-11 h-6 rounded-full transition-colors relative ${darkMode ? 'bg-emerald-500' : 'bg-gray-300'}`}>
              <span className={`absolute top-1 w-4 h-4 rounded-full transition-all bg-white ${darkMode ? 'left-6' : 'left-1'}`}></span>
            </button>
            <span className="text-xs text-gray-400">🌙</span>
          </div>
          <button onClick={() => setShowCreateModal(true)}
            className="bg-emerald-500 hover:bg-emerald-400 text-gray-950 font-bold text-xs uppercase tracking-widest px-4 py-2 rounded-lg transition">
            + New Issue
          </button>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {Object.entries(counts).map(([status, count]) => (
            <div key={status} className={`${card} border rounded-xl p-4 cursor-pointer transition hover:border-emerald-500/30`}
              onClick={() => setFilterStatus(status)}>
              <div className="flex items-center justify-between mb-2">
                <span className={`text-xs uppercase tracking-widest ${muted}`}>{status}</span>
                <span className={statusConfig[status]?.icon ? statusConfig[status].color : 'text-emerald-400'}>
